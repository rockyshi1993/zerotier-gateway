...
首版不做复杂 VPN 平台，不默认配置 Exit Node，不做自建 Controller、Moon/Private Root 或多网络编排。
...
1. 获取仓库内容，进入项目目录。
2. 复制 `config/example.env` 为项目根目录下的 `.env`，并填写你的 ZeroTier Network ID、三台机器的 ZeroTier IP、代理账号密码。
...
<details>
<summary>旧版 README（历史参考，不作为当前主线）</summary>

## 旧版 README：ZeroTier Gateway 一键配置脚本
