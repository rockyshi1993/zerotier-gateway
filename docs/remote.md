...
规则预览里应该看到两类来源：对端 Windows 的 ZeroTier IP 用于直连，初始化脚本中选择的 Ubuntu IP 用于中转。默认第一台中转服务器是 `10.246.77.1`；如果改用新服务器，例如 `10.246.77.2`，重新运行 `init-config.ps1`，一路回车保留现值、只更新 Ubuntu IP，再重跑防火墙脚本。
